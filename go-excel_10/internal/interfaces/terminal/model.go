...
func (m *Model) View() string {
	...
	// 1️⃣ Calcular ancho máximo de cada columna según contenido y encabezado
	colWidths := make([]int, m.Sheet.Cols)
	for j := 0; j < m.Sheet.Cols; j++ {
		// Empezamos con el ancho del encabezado
		colWidths[j] = len(string('A' + j))
	}
	...
	// 2️⃣ Obtener ancho de la terminal
	termWidth, _, err := terminalSize()
	if err != nil {
		termWidth = 80 // ancho por defecto
	}

	// 3️⃣ Ajustar columnas si exceden ancho de la terminal
