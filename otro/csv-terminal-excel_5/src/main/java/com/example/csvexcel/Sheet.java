...
//`````````````````````````````````````````````````````````````````
   ...
   //`````````````````````````````````````````````````````````````````
    ...
    public void filterByColumn(int columnIndex, String condition) {
        // Extraer operador y valor del filtro
        String operator = "";
        double value = 0.0;

        if (condition.startsWith(">=")) {
            operator = ">=";
            value = Double.parseDouble(condition.substring(2));
        } else if (condition.startsWith("<=")) {
            operator = "<=";
            value = Double.parseDouble(condition.substring(2));
        } else if (condition.startsWith(">")) {
            operator = ">";
            value = Double.parseDouble(condition.substring(1));
        } else if (condition.startsWith("<")) {
            operator = "<";
            value = Double.parseDouble(condition.substring(1));
        } else if (condition.startsWith("==")) {
        ...
        System.out.println("\nFiltrando columna " + columnIndex + " con condición " + operator + value);
        ...
        for (String[] row : rows) {
            ...
            try {
                double cellValue = Double.parseDouble(row[columnIndex]);
