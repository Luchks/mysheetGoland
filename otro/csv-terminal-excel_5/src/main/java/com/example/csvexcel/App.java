...
public class App {
    ...
    public static void main(String[] args) throws Exception { // <-- throws Exception
        ...
        while (true) {
            ...
            System.out.println("\nTeclas: h/j/k/l=Mover | e=Editar | s=Guardar | c/C=Agregar Columna | d=Eliminar Columna | f=Filtrar | q=Salir");
            ...
            switch (key) {
                ...
                case 'f': // filtrar por columna actual
                    terminal.writer().print("Filtrar columna " + curCol + " (ej: >30, <100, ==Peru): ");
                    terminal.writer().flush();
                    ...
                    while (true) {
                        key = terminal.reader().read();
                        if (key == 10 || key == 13) break; // Enter
                        if (key == 127 && filterInput.length() > 0) { // Backspace
                    ...
                    terminal.writer().println("Presiona Enter para continuar...");
                    terminal.writer().flush();
                    terminal.reader().read(); // esperar Enter
                    break;
                case 'q':
                    terminal.close();
                    return;
