   ...
   //`````````````````````````````````````````````````````````````````
    ...
    public void filterByColumn(int columnIndex, String condition) {
        if (originalRows == null) {
            originalRows = new ArrayList<>(rows); // guardamos el original la primera vez
        }

        List<String[]> filtered = new ArrayList<>();

        String operator = "";
        String valueStr = condition.replaceAll("^[><=!]+", ""); // valor sin operador
        double valueNum = 0;
        boolean isNumeric = valueStr.matches("-?\\d+(\\.\\d+)?");

        if (isNumeric) {
            valueNum = Double.parseDouble(valueStr);
        }
        ...
        // Sustituimos las filas actuales por las filtradas
        rows.clear();
